...
func (client *sqliteClient) UpdateVPCIndex(provider, vpcID, resourceId, resourceType string) error {
...
func (client *sqliteClient) SyncVPCIndexes() error {
...
func (client *sqliteClient) PutVPCIndex(vpcIndex *types.VPCIndex) error {
	key := vpcIndex.DbId()
	//fmt.Printf("DEBUG: Putting VPCIndex with key: %s, value: %+v\n", key, vpcIndex)
	err := client.putObject(vpcIndex, key, vpcIndexTable)
	if err != nil {
...
func (client *sqliteClient) GetVPCIndex(id string) (*types.VPCIndex, error) {
	//fmt.Printf("DEBUG: Calling get for VPCIndex with key: %s\n", id)
	vpcIndex := &types.VPCIndex{}
	err := client.getObject(id, vpcIndexTable, vpcIndex)
	if err != nil {
...
func (client *sqliteClient) ListVPCIndex() ([]*types.VPCIndex, error) {
	objects, err := client.listObjects(vpcIndexTable, func() interface{} { return &types.VPCIndex{} })
	if err != nil {
		return nil, err
	}

	vpcIndexes := make([]*types.VPCIndex, len(objects))
	for i, obj := range objects {
		vpcIndexes[i] = obj.(*types.VPCIndex)
	}
	return vpcIndexes, nil
}

func (client *sqliteClient) DeleteVPCIndex(id string) error {
	return client.deleteObject(id, vpcIndexTable)
}

// New function to call SyncVPCIndexes after all resource sync is complete.
func (client *sqliteClient) FinalizeSync() error {
