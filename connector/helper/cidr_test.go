...
func TestCIDRContainsCIDR(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "Cidr A being nil returns false",
			cidrA: nil,
			cidrB: parseCIDR("10.0.0.0/24"),
			want:  false,
		},
		{
			name:  "Cidr B being nil returns false",
			cidrA: parseCIDR("10.0.0.0/24"),
			cidrB: nil,
			want:  false,
		},
		{
			name:  "Cidr A and B being nil returns false",
			cidrA: nil,
			cidrB: nil,
			want:  false,
		},
...
func TestCIDRsOverlap(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "Nil cidrA will return false",
			cidrA: nil,
			cidrB: parseCIDR("192.168.0.0/16"),
			want:  false,
		},
		{
			name:  "Nil cidrB will return false",
			cidrA: parseCIDR("192.168.0.0/16"),
			cidrB: nil,
			want:  false,
		},
		{
			name:  "Both nil CIDRs will return false",
			cidrA: nil,
			cidrB: nil,
			want:  false,
		},
...
func Test_getBiggerCIDR(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "when first cidr is nil, the second is returned",
			cidr1: nil,
			cidr2: parseCIDR("192.168.1.0/24"),
			want:  parseCIDR("192.168.1.0/24"),
		},
		{
			name:  "when second cidr is nil, the first is returned",
			cidr1: parseCIDR("192.168.1.0/24"),
			cidr2: nil,
			want:  parseCIDR("192.168.1.0/24"),
		},
		{
			name:  "when both cidrs are nil, the nil is returned",
			cidr1: nil,
			cidr2: nil,
			want:  nil,
		},
