...
get_vm_status() {
    gcloud compute instances describe "$SCRIPT_VM_NAME" --zone="$SCRIPT_SUBREGION" \
...
while true; do
    ...
    SCRIPT_VM_STATUS=$(get_vm_status)

    if [ "$SCRIPT_VM_STATUS" == "RUNNING" ]; then
        echo "VM is running."
        break
    else
        echo "Waiting for VM to be running... Current status: $SCRIPT_VM_STATUS"
        sleep 10
    fi
done
