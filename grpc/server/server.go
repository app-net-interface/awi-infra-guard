...
func (s *Server) ListVpcConnections(ctx context.Context, in *infrapb.ListVpcConnectionsRequest) (*infrapb.ListVpcConnectionsResponse, error) {
	cloudProvider, err := s.strategy.GetProvider(ctx, in.Provider)
	if err != nil {
		return nil, err
	}

	connections, err := cloudProvider.ListVpcConnections(ctx, in)
	if err != nil {
		return &infrapb.ListVpcConnectionsResponse{
			Err: &infrapb.Error{
				Code:         500,
				ErrorMessage: err.Error(),
				Serverity:    "ERROR",
	...
	ptrConns := make([]*types.VPCConnection, len(connections))
	for i := range connections {
		ptrConns[i] = &connections[i]
	}

	return &infrapb.ListVpcConnectionsResponse{
		Connections: typesVpcConnectionsToGrpc(ptrConns),
	}, nil
}
